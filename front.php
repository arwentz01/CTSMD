...
if ($route === '/messages/thread') {
    ...
    if ($conversationId > 0) CommunicationReadStateService::markConversationRead($dbForAuth, (int)$currentAuthUser['id'], (int)$conversationId);
}
if ($route === '/channels/view') {
    $channelId = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT) ?: 0;
    if ($channelId > 0 && CommunicationReadStateService::canAccessChannel($dbForAuth, $currentAuthUser, (int)$channelId)) CommunicationReadStateService::markChannelRead($dbForAuth, (int)$currentAuthUser['id'], (int)$channelId);
}
...
if (VisualPass3::handles($route)) {$data = require __DIR__ . '/src/mock-data.php';VisualPass3::render($route, $detectedBasePath, $data);}
require_once __DIR__ . '/src/VisualPass.php';
if (VisualPass::handles($route)) {$data = require __DIR__ . '/src/mock-data.php';VisualPass::render($route, $detectedBasePath, $data);}
require __DIR__ . '/index.php';
