...
final class FormManagementExperience
{
    ...
    private static function assign(PDO $db, array $actor, int $formId, array $input): int
    {
        ...
        $context = (string)($input['assignment_context'] ?? 'organization');
        ...
        $productionId = null;
        if ($form['production_id'] !== null) {
            $productionId = (int)$form['production_id'];
            $selected = ProductionContext::selected($db, $actor);
            ...
            $selected = ProductionContext::selected($db, $actor);
            if (!$selected) throw new RuntimeException('Select an active working production before assigning this form in production context.');
            $productionId = (int)$selected['id'];
        } elseif ($context !== 'organization') {
            throw new RuntimeException('Choose a valid assignment context.');
        }
        ...
        try {
            ...
            self::audit($db,(int)$actor['id'],'form.assignments_created','form',$formId,'Assigned form to audience.',['assignment_context'=>$context,'production_id'=>$productionId,'audience'=>$audience,'requested_user_ids'=>$userIds,'created_count'=>$created,'due_at'=>$dueAt]);
            $db->commit();
            return $created;
        } catch (Throwable $e) {
            if ($db->inTransaction()) $db->rollBack();
    ...
    private static function resolveAudience(PDO $db, ?int $productionId, string $audience, array $selectedIds): array
    {
        ...
        if (!in_array($audience, $allowed, true)) throw new RuntimeException('Choose an audience that matches the selected assignment context.');
