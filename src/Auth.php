...
final class Auth
{
    ...
    public static function currentUser(PDO $db): ?array
    {
        ...
        $stmt = $db->prepare("SELECT id,first_name,last_name,email,initials,display_role AS role,active,account_status,last_login_at FROM users WHERE id=:id AND active=1 AND account_status='active' LIMIT 1");
        ...
        $user['roles'] = self::roles($db,$userId);
        $user['permissions'] = self::permissions($db,$userId);
        return $user;
    }
    ...
    {
        if(!self::localIdentitySwitchEnabled()) throw new RuntimeException('Local identity switching is disabled.');
        $stmt=$db->prepare('SELECT id FROM users WHERE id=:id AND active=1 LIMIT 1');$stmt->execute(['id'=>$userId]);
        if(!$stmt->fetchColumn()) throw new RuntimeException('That local test identity is unavailable.');
        self::startSession();session_regenerate_id(true);$_SESSION[self::SESSION_USER_ID]=$userId;$_SESSION['auth_local_identity']=true;
    }

    public static function logout(): void
    {
        self::startSession();
        unset($_SESSION[self::SESSION_USER_ID],$_SESSION['auth_authenticated_at'],$_SESSION['auth_local_identity']);
    ...
    {
        $stmt=$db->prepare('SELECT id,email,active FROM users WHERE id=:id LIMIT 1');$stmt->execute(['id'=>$userId]);$user=$stmt->fetch();
        if(!$user || !(bool)$user['active'] || empty($user['email'])) throw new RuntimeException('That person needs an active account with an email address before invitation.');
        $token=bin2hex(random_bytes(32));$hash=hash('sha256',$token);$expires=(new DateTimeImmutable('+'.max(1,$hours).' hours'))->format('Y-m-d H:i:s');
        $db->prepare('UPDATE auth_invitations SET accepted_at=COALESCE(accepted_at,CURRENT_TIMESTAMP) WHERE user_id=:user AND accepted_at IS NULL')->execute(['user'=>$userId]);
        $db->prepare('INSERT INTO auth_invitations (user_id,token_hash,expires_at,created_by_user_id) VALUES (:user,:hash,:expires,:creator)')->execute(['user'=>$userId,'hash'=>$hash,'expires'=>$expires,'creator'=>$creatorId]);
        $db->prepare("UPDATE users SET account_status='invited' WHERE id=:id AND password_hash IS NULL")->execute(['id'=>$userId]);
        return $token;
    }
    ...
    public static function invitation(PDO $db,string $token): ?array
    {
        ...
        $s=$db->prepare("SELECT i.id,i.user_id,i.expires_at,u.first_name,u.last_name,u.email FROM auth_invitations i JOIN users u ON u.id=i.user_id AND u.active=1 WHERE i.token_hash=:hash AND i.accepted_at IS NULL AND i.expires_at>NOW() LIMIT 1");$s->execute(['hash'=>$hash]);return $s->fetch()?:null;
    }

    public static function acceptInvitation(PDO $db,string $token,string $password): int
    {
        self::assertPassword($password);$inv=self::invitation($db,$token);if(!$inv)throw new RuntimeException('That invitation is invalid or has expired.');
        $hash=password_hash($password,PASSWORD_DEFAULT);$db->beginTransaction();try{$db->prepare("UPDATE users SET password_hash=:password,account_status='active',password_changed_at=CURRENT_TIMESTAMP WHERE id=:id")->execute(['password'=>$hash,'id'=>(int)$inv['user_id']]);$db->prepare('UPDATE auth_invitations SET accepted_at=CURRENT_TIMESTAMP WHERE id=:id')->execute(['id'=>(int)$inv['id']]);$db->commit();return (int)$inv['user_id'];}catch(Throwable $e){if($db->inTransaction())$db->rollBack();throw $e;}
    }

    public static function createPasswordReset(PDO $db,string $email): ?string
    {
        $s=$db->prepare("SELECT id FROM users WHERE LOWER(email)=:email AND active=1 AND account_status='active' LIMIT 1");$s->execute(['email'=>mb_strtolower(trim($email))]);$id=(int)($s->fetchColumn()?:0);if(!$id)return null;
        $token=bin2hex(random_bytes(32));$hash=hash('sha256',$token);$db->prepare('UPDATE auth_password_resets SET used_at=COALESCE(used_at,CURRENT_TIMESTAMP) WHERE user_id=:user AND used_at IS NULL')->execute(['user'=>$id]);$db->prepare("INSERT INTO auth_password_resets (user_id,token_hash,expires_at) VALUES (:user,:hash,DATE_ADD(NOW(),INTERVAL 2 HOUR))")->execute(['user'=>$id,'hash'=>$hash]);return $token;
    }

    public static function resetPassword(PDO $db,string $token,string $password): void
    {
        self::assertPassword($password);if(!preg_match('/^[a-f0-9]{64}$/',$token))throw new RuntimeException('That reset link is invalid or expired.');$hash=hash('sha256',$token);
        $db->beginTransaction();try{$s=$db->prepare("SELECT id,user_id FROM auth_password_resets WHERE token_hash=:hash AND used_at IS NULL AND expires_at>NOW() FOR UPDATE");$s->execute(['hash'=>$hash]);$row=$s->fetch();if(!$row)throw new RuntimeException('That reset link is invalid or expired.');$db->prepare("UPDATE users SET password_hash=:password,password_changed_at=CURRENT_TIMESTAMP,account_status='active' WHERE id=:user")->execute(['password'=>password_hash($password,PASSWORD_DEFAULT),'user'=>(int)$row['user_id']]);$db->prepare('UPDATE auth_password_resets SET used_at=CURRENT_TIMESTAMP WHERE id=:id')->execute(['id'=>(int)$row['id']]);$db->commit();}catch(Throwable $e){if($db->inTransaction())$db->rollBack();throw $e;}
    }

    public static function localIdentitySwitchEnabled(): bool
    {
        $environment=strtolower((string)(getenv('APP_ENV')?:($_ENV['APP_ENV']??'production')));if($environment==='local')return true;
        $remote=(string)($_SERVER['REMOTE_ADDR']??'');$host=strtolower((string)($_SERVER['HTTP_HOST']??''));$host=preg_replace('/:\\d+$/','',$host)?:$host;
