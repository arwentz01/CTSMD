...
final class PushService
{
    ...
    {
        $payload=json_encode(['title'=>$notification['title'],'body'=>$notification['body'],'url'=>$notification['action_path']?:'/app','tag'=>$notification['tag']?:('ctsmd-'.$notification['id']),'badgeCount'=>1],JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR);
        ...
        $endpoint=(string)$sub['endpoint'];$origin=(string)(parse_url($endpoint,PHP_URL_SCHEME).'://'.parse_url($endpoint,PHP_URL_HOST));$jwt=self::vapidJwt($origin);$headers=['Content-Type: application/octet-stream','Content-Encoding: aes128gcm','TTL: 86400','Urgency: '.($notification['urgency']?:'normal'),'Authorization: vapid t='.$jwt.', k='.self::publicKey(),'Content-Length: '.strlen($body)];
