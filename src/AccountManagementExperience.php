...
final class AccountManagementExperience
{
    ...
    private static function handlePost(PDO $db,array $actor,string $basePath,string $route):never
    {
        ...
        try{
            ...
            if($action==='invite'){
                $token=Auth::createInvitation($db,(int)$id,(int)$actor['id']);$path='/activate?token='.$token;$_SESSION['accounts_invite_link']=self::absoluteUrl($basePath.$path);self::audit($db,(int)$actor['id'],'account.invited',$id,'Issued account invitation.');self::flash('success','Invitation created. Copy the one-time activation link below.');
    ...
    private static function absoluteUrl(string $path):string{$scheme=(!empty($_SERVER['HTTPS'])&&$_SERVER['HTTPS']!=='off')?'https':'http';$host=(string)($_SERVER['HTTP_HOST']??'localhost');return $scheme.'://'.$host.$path;}
