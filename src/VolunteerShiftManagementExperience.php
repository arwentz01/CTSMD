...
final class VolunteerShiftManagementExperience
{
    ...
    public static function render(string $route, string $basePath): never
    {
        ...
        if ($route === '/admin/volunteer-shifts/view') {
            ...
            $shift = self::shift($db, (int)$id, $productionId);
        }
    ...
    private static function handlePost(PDO $db, string $route, string $basePath, array $user): never
    {
        ...
        try {
            if ($action === 'create') {
                ...
                self::flash('success', 'Volunteer shift created in the selected production.');
            ...
            if ($action === 'roster_status') {
                ...
                self::updateRosterStatus($db, $user, (int)$signupId, (int)$shiftId, (string)($_POST['status'] ?? ''));
    ...
    {
        $production = ProductionContext::selected($db, $actor);
        if (!$production) {
            throw new RuntimeException('Select an active production before creating or editing volunteer shifts.');
        }
        ...
        try {
            ...
            if ($shiftId > 0) {
                ...
                if ((int)$before['production_id'] !== (int)$production['id']) throw new RuntimeException('That shift belongs to a different production workspace.');
    ...
    private static function updateRosterStatus(PDO $db, array $actor, int $signupId, int $shiftId, string $status): void
    {
        ...
        if ($signupId < 1 || $shiftId < 1 || !in_array($status, $allowed, true)) {
            throw new RuntimeException('Choose a valid roster status.');
        }

        $production = ProductionContext::selected($db, $actor);
        if (!$production) {
            throw new RuntimeException('Select an active production before changing a volunteer roster.');
        }

        $db->beginTransaction();
        try {
            $stmt = $db->prepare("SELECT s.id, s.shift_id, s.user_id, s.status, vs.title, vs.production_id, CONCAT(u.first_name, ' ', u.last_name) AS volunteer_name FROM volunteer_shift_signups s JOIN volunteer_shifts vs ON vs.id = s.shift_id JOIN users u ON u.id = s.user_id WHERE s.id = :id FOR UPDATE");
            $stmt->execute(['id' => $signupId]);
            $signup = $stmt->fetch();
            if (!$signup || (int)$signup['shift_id'] !== $shiftId) throw new RuntimeException('That roster entry no longer exists.');
            if ((int)$signup['production_id'] !== (int)$production['id']) throw new RuntimeException('That roster entry belongs to a different production workspace.');
            ...
            self::audit($db, (int)$actor['id'], 'volunteer.roster_status_changed', 'volunteer_shift_signup', $signupId, 'Updated volunteer shift roster status.', [
                'production_id' => (int)$signup['production_id'],
    ...
    private static function shifts(PDO $db, int $productionId): array
    {
    ...
    private static function shift(PDO $db, int $shiftId, int $productionId): ?array
    {
        if ($shiftId < 1 || $productionId < 1) return null;
        $stmt = $db->prepare('SELECT vs.*, p.title AS production_title FROM volunteer_shifts vs LEFT JOIN productions p ON p.id = vs.production_id WHERE vs.id = :id AND vs.production_id = :production_id LIMIT 1');
        $stmt->execute(['id' => $shiftId, 'production_id' => $productionId]);
        $shift = $stmt->fetch();
    ...
    private static function page(PDO $db, string $route, string $basePath, array $user, ?array $production, array $requirements, ?array $shift): never
    {
        ...
        $flash = $_SESSION['volunteer_shift_admin_flash'] ?? $_SESSION['production_context_flash'] ?? null;
        unset($_SESSION['volunteer_shift_admin_flash'], $_SESSION['production_context_flash']);
        $shifts = self::shifts($db, $production ? (int)$production['id'] : 0);
...
<section class="vsm-hero"><div><small><?= $esc(strtoupper($production['title'])) ?> · WORKING PRODUCTION</small><h2>Staff the work behind the show.</h2><p>Only shifts belonging to this selected production appear here.</p></div><a class="button" href="<?= $url('/admin/volunteer-shifts/new') ?>">Create shift</a></section>
...
<?php else: ?>
<section class="vsm-head"><div><small><?= $esc(strtoupper($production['title'])) ?> · WORKING PRODUCTION</small><h2><?= $editing ? $esc($shift['title']) : 'Create volunteer shift' ?></h2><p><?= $editing ? 'Edit the opportunity and manage its confirmed roster.' : 'Define the opportunity, capacity, approval gate, and requirements.' ?></p></div><a href="<?= $url('/admin/volunteer-shifts') ?>">← All shifts</a></section>
<?php if ($route === '/admin/volunteer-shifts/view' && !$shift): ?><section class="vsm-empty"><b>Shift not found in this production</b><p>It may belong to another active production. Switch the working production and try again.</p></section><?php else: ?>
