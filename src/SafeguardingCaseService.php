...
final class SafeguardingCaseService
{
    ...
    {
        return $db->query("SELECT u.id,CONCAT(u.first_name,' ',u.last_name) name,u.last_name,u.first_name FROM users u JOIN auth_user_roles aur ON aur.user_id=u.id JOIN auth_roles ar ON ar.id=aur.role_id AND ar.active=1 LEFT JOIN auth_role_permissions arp ON arp.role_id=ar.id LEFT JOIN auth_permissions ap ON ap.id=arp.permission_id WHERE u.active=1 AND u.account_status<>'disabled' AND (ar.code IN ('administrator','safeguarding') OR ap.code='safeguarding.manage') GROUP BY u.id,u.first_name,u.last_name ORDER BY u.last_name,u.first_name")->fetchAll();
    }
    ...
    private static function isSafeguardingUser(PDO $db,int $userId):bool{$s=$db->prepare("SELECT 1 FROM users u JOIN auth_user_roles aur ON aur.user_id=u.id JOIN auth_roles ar ON ar.id=aur.role_id AND ar.active=1 LEFT JOIN auth_role_permissions arp ON arp.role_id=ar.id LEFT JOIN auth_permissions ap ON ap.id=arp.permission_id WHERE u.id=:user AND u.active=1 AND u.account_status<>'disabled' AND (ar.code IN ('administrator','safeguarding') OR ap.code='safeguarding.manage') LIMIT 1");$s->execute(['user'=>$userId]);return(bool)$s->fetchColumn();}
