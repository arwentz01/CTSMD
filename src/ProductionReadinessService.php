...
final class ProductionReadinessService
{
    public static function build(PDO $db,array $user):array
    {
        ...
        $signals=[];
        $students=self::count($db,"SELECT COUNT(DISTINCT user_id) FROM production_memberships WHERE production_id=? AND audience_type='student' AND status='active'",[$id]);
        $guardiansMissing=self::count($db,"SELECT COUNT(*) FROM production_memberships pm WHERE pm.production_id=? AND pm.audience_type='student' AND pm.status='active' AND NOT EXISTS (SELECT 1 FROM family_relationships fr JOIN production_memberships gm ON gm.production_id=pm.production_id AND gm.user_id=fr.guardian_user_id AND gm.audience_type='guardian' AND gm.status='active' WHERE fr.student_user_id=pm.user_id AND fr.status='active')",[$id]);
        ...
        $forms=self::count($db,"SELECT COUNT(*) FROM form_assignments fa JOIN forms f ON f.id=fa.form_id AND f.active=1 WHERE fa.production_id=? AND fa.status<>'completed'",[$id]);
    ...
    private static function staff(PDO $db,int $productionId):array{$s=$db->prepare("SELECT DISTINCT u.id,CONCAT(u.first_name,' ',u.last_name) name FROM production_memberships pm JOIN users u ON u.id=pm.user_id AND u.active=1 WHERE pm.production_id=:production AND pm.audience_type='staff' AND pm.status='active' ORDER BY u.last_name,u.first_name");$s->execute(['production'=>$productionId]);return $s->fetchAll();}
    private static function staffMember(PDO $db,int $productionId,int $userId):bool{$s=$db->prepare("SELECT 1 FROM production_memberships WHERE production_id=:production AND user_id=:user AND audience_type='staff' AND status='active' LIMIT 1");$s->execute(['production'=>$productionId,'user'=>$userId]);return (bool)$s->fetchColumn();}
