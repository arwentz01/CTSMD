...
final class ProductionPeopleExperience
{
    ...
    private static function page(string $basePath, array $user, ?array $production, array $members, array $candidates, array $coverage): never
    {
        ...
        $subnav = AppNavigation::productionSubnav('/production/people', $user);
        ...
        <div class="production-people-layout"><section class="production-people-panel"><header><div><small>ACTIVE ROSTER</small><h3>Production membership</h3></div><span class="badge badge-count"><?= count($active) ?> live</span></header>
        <div class="production-people-table op-table" aria-label="Production membership"><div class="production-people-columns op-table-columns"><span>Person</span><span>Audience</span><span>Production role</span><span>Account role</span><span>Actions</span></div><?php foreach (['student' => 'Students / Cast', 'guardian' => 'Guardians', 'staff' => 'Production Staff'] as $type => $label): ?><div class="op-table-group production-people-group"><header class="op-table-group-header"><div><b><?= $esc($label) ?></b><small><?php $group = array_values(array_filter($active, static fn(array $m): bool => $m['audience_type'] === $type)); ?><?= count($group) ?> active</small></div></header><?php if (!$group): ?><div class="op-table-empty">No active <?= $esc($label) ?>.</div><?php else: foreach ($group as $member): ?><article class="production-member op-table-row"><div class="production-member-id op-table-primary"><i><?= $esc($member['initials']) ?></i><div><b><?= $esc($member['name']) ?></b><small><?= $esc((string)$member['account_status']) ?></small></div></div><div><span class="op-mobile-label">Audience</span><span class="badge badge-compact badge-neutral"><?= $esc(ucfirst((string)$member['audience_type'])) ?></span></div><div><span class="op-mobile-label">Production role</span><?= $esc((string)($member['participation_role'] ?: 'Not set')) ?></div><div><span class="op-mobile-label">Account role</span><?= $esc($member['account_role']) ?></div><form class="op-table-actions" method="post"><input type="hidden" name="csrf_token" value="<?= $esc((string)$_SESSION['production_people_csrf']) ?>"><input type="hidden" name="action" value="remove"><input type="hidden" name="membership_id" value="<?= (int)$member['id'] ?>"><button class="btn btn-tertiary btn-compact" type="submit">Remove</button></form></article><?php endforeach; endif; ?></div><?php endforeach; ?>
        <?php if($disabledActive):?><div class="op-table-group production-people-group"><header class="op-table-group-header"><div><b>Disabled accounts · cleanup</b><small><?= count($disabledActive) ?> retained</small></div></header><?php foreach($disabledActive as $member):?><article class="production-member op-table-row is-inactive"><div class="production-member-id op-table-primary"><i><?= $esc($member['initials']) ?></i><div><b><?= $esc($member['name']) ?></b><small>Account disabled</small></div></div><div><span class="op-mobile-label">Audience</span><span class="badge badge-compact badge-danger"><?= $esc(ucfirst((string)$member['audience_type'])) ?></span></div><div><span class="op-mobile-label">Production role</span><?= $esc((string)($member['participation_role'] ?: 'Not set')) ?></div><div><span class="op-mobile-label">Account role</span><?= $esc($member['account_role']) ?></div><form class="op-table-actions" method="post"><input type="hidden" name="csrf_token" value="<?= $esc((string)$_SESSION['production_people_csrf']) ?>"><input type="hidden" name="action" value="remove"><input type="hidden" name="membership_id" value="<?= (int)$member['id'] ?>"><button class="btn btn-tertiary btn-compact" type="submit">Remove</button></form></article><?php endforeach;?></div><?php endif;?></div>
        </section>
