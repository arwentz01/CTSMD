...
{
    private const ROUTE='/volunteer/service-record';
    ...
    private static function page(string $basePath,array $user,array $record,bool $print):never
    {
        ...
        $person=$record['person'];$sub=[['label'=>'Readiness','href'=>'/volunteer-readiness','active'=>false],['label'=>'Opportunities','href'=>'/volunteer-shifts','active'=>false],['label'=>'Service record','href'=>self::ROUTE,'active'=>true],['label'=>'Training','href'=>'/volunteer/training','active'=>false],['label'=>'Hours','href'=>'/volunteer/history','active'=>false]];
        ...
        <section class="vsr-panel wide ledger"><header><div><small>VERIFICATION LEDGER</small><h2>Recent verified service entries</h2></div><?php if(!$print):?><a href="<?=$u('/volunteer/history')?>">View full hours ledger →</a><?php endif;?></header><?php foreach(array_slice($record['hours'],0,$print?100:12) as $h):?><article><div><b><?=$e((string)($h['shift_title']?:'Verified service'))?></b><small><?=date('M j, Y',strtotime((string)$h['served_at']))?> · <?=$e((string)($h['production_title']?:'CTSMD'))?><?=$h['shift_category']?' · '.$e((string)$h['shift_category']):''?></small></div><strong><?=$hours((int)$h['minutes'])?>h</strong></article><?php endforeach;?></section>
