...
final class ModerationExperience
{
    ...
    {
        $term=trim((string)($input['term']??''));$category=trim((string)($input['category']??'profanity'));$action=(string)($input['rule_action']??'review');$mode=(string)($input['match_mode']??'normalized');$severity=(string)($input['severity']??'medium');$notes=trim((string)($input['notes']??''));$aliases=self::aliases((string)($input['aliases']??''));
        ...
        $db->beginTransaction();
        try{
            if($id){if(!self::term($db,$id)) throw new RuntimeException('That moderation rule no longer exists.');$stmt=$db->prepare('UPDATE moderation_terms SET term=:term,category=:category,action=:action,match_mode=:mode,severity=:severity,aliases_json=:aliases,notes=:notes,updated_by_user_id=:actor WHERE id=:id');$stmt->execute(['term'=>$term,'category'=>$category,'action'=>$action,'mode'=>$mode,'severity'=>$severity,'aliases'=>$aliases?json_encode($aliases,JSON_THROW_ON_ERROR):null,'notes'=>$notes?:null,'actor'=>(int)$user['id'],'id'=>$id]);self::audit($db,(int)$user['id'],'moderation.term_updated','moderation_term',$id,'Updated a Community moderation rule.',['category'=>$category,'action'=>$action,'match_mode'=>$mode,'severity'=>$severity]);}
            else{$stmt=$db->prepare('INSERT INTO moderation_terms (term,category,action,match_mode,severity,aliases_json,notes,active,created_by_user_id,updated_by_user_id) VALUES (:term,:category,:action,:mode,:severity,:aliases,:notes,1,:actor,:actor)');$stmt->execute(['term'=>$term,'category'=>$category,'action'=>$action,'mode'=>$mode,'severity'=>$severity,'aliases'=>$aliases?json_encode($aliases,JSON_THROW_ON_ERROR):null,'notes'=>$notes?:null,'actor'=>(int)$user['id']]);$id=(int)$db->lastInsertId();self::audit($db,(int)$user['id'],'moderation.term_created','moderation_term',$id,'Created a Community moderation rule.',['category'=>$category,'action'=>$action,'match_mode'=>$mode,'severity'=>$severity]);}
            $db->commit(); return $id;
        }catch(Throwable $e){if($db->inTransaction())$db->rollBack();if($e instanceof RuntimeException)throw $e;if($e instanceof PDOException && str_contains($e->getMessage(),'Duplicate')) throw new RuntimeException('That moderation term already exists.');throw new RuntimeException('The moderation rule could not be saved.');}
    }

    private static function toggleTerm(PDO $db,array $user,int $id): void
    {
        $term=self::term($db,$id); if(!$term) throw new RuntimeException('That moderation rule could not be found.');$active=(int)$term['active']===1?0:1;$stmt=$db->prepare('UPDATE moderation_terms SET active=:active,updated_by_user_id=:actor WHERE id=:id');$stmt->execute(['active'=>$active,'actor'=>(int)$user['id'],'id'=>$id]);self::audit($db,(int)$user['id'],$active?'moderation.term_activated':'moderation.term_deactivated','moderation_term',$id,$active?'Activated a Community moderation rule.':'Deactivated a Community moderation rule.',['active'=>(bool)$active]);
    }

    private static function reviewPost(PDO $db,array $user,int $postId,bool $approve): void
    {
        if($postId<1) throw new RuntimeException('That post could not be found.');$db->beginTransaction();
        try{$stmt=$db->prepare("SELECT cp.id,cp.moderation_status,cp.channel_id,cp.hidden_at,cp.deleted_at FROM channel_posts cp WHERE cp.id=:id FOR UPDATE");$stmt->execute(['id'=>$postId]);$post=$stmt->fetch();if(!$post||$post['moderation_status']!=='pending'||$post['hidden_at']!==null||$post['deleted_at']!==null) throw new RuntimeException('Only visible posts awaiting review can be moderated.');$status=$approve?'published':'rejected';$update=$db->prepare('UPDATE channel_posts SET moderation_status=:status,moderated_by_user_id=:actor,moderated_at=CURRENT_TIMESTAMP WHERE id=:id');$update->execute(['status'=>$status,'actor'=>(int)$user['id'],'id'=>$postId]);self::audit($db,(int)$user['id'],$approve?'moderation.post_approved':'moderation.post_rejected','channel_post',$postId,$approve?'Approved a held Community post.':'Rejected a held Community post.',['channel_id'=>(int)$post['channel_id'],'result'=>$status]);$db->commit();}catch(Throwable $e){if($db->inTransaction())$db->rollBack();if($e instanceof RuntimeException)throw $e;throw new RuntimeException('The moderation decision could not be saved.');}
    }

    private static function terms(PDO $db): array{return $db->query("SELECT mt.*,CONCAT(c.first_name,' ',c.last_name) creator,CONCAT(u.first_name,' ',u.last_name) updater FROM moderation_terms mt LEFT JOIN users c ON c.id=mt.created_by_user_id LEFT JOIN users u ON u.id=mt.updated_by_user_id ORDER BY mt.active DESC,FIELD(mt.severity,'critical','high','medium','low'),mt.category,mt.term")->fetchAll();}
