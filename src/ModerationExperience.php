...
final class ModerationExperience
{
    ...
    private static function termRow(array $term,string $basePath,string $csrf,callable $esc): void
    {
        ...
        $severityTone=match($severity){'critical'=>'badge-danger is-danger','high','medium'=>'badge-warning is-warning',default=>'badge-neutral is-neutral'};
        ?>
        <div class="mod-table-row op-table-row<?= !$term['active']?' is-inactive':'' ?>">
            <div class="mod-table-term op-table-primary"><b><?= $esc((string)$term['term']) ?></b><?php if(!empty($term['notes'])):?><small><?= $esc((string)$term['notes']) ?></small><?php endif;?></div>
            <div><span class="mod-mobile-label op-mobile-label">Severity</span><span class="mod-severity op-table-status badge badge-compact <?= $esc($severity) ?> <?= $severityTone ?>"><?= $esc(ucfirst($severity)) ?></span></div>
            <div><span class="mod-mobile-label op-mobile-label">Action</span><?= $term['action']==='block'?'Block immediately':'Hold for review' ?></div>
            <div><span class="mod-mobile-label op-mobile-label">Matching</span><?= $term['match_mode']==='normalized'?'Normalized':'Exact' ?></div>
            <div><span class="mod-status op-table-status badge badge-compact <?= $term['active']?'is-success badge-success':'is-neutral badge-neutral' ?>"><?= $term['active']?'Active':'Inactive' ?></span></div>
            <div class="mod-table-actions op-table-actions"><a class="btn btn-tertiary btn-compact" href="<?= $url('/admin/moderation/terms/edit?id='.(int)$term['id']) ?>">Edit</a><form method="post"><input type="hidden" name="csrf_token" value="<?= $esc($csrf) ?>"><input type="hidden" name="action" value="toggle_term"><input type="hidden" name="term_id" value="<?= (int)$term['id'] ?>"><button class="btn btn-tertiary btn-compact"><?= $term['active']?'Deactivate':'Activate' ?></button></form></div>
        </div>
        <?php
    }
    ...
    private static function page(string $route,string $basePath,array $user,PDO $db,?array $edit): never
    {
        ...
        <?php if($flash):?><div class="mod-flash alert <?= $flash['type']==='success'?'alert-success':'alert-danger' ?> alert-compact <?= $esc($flash['type']) ?>"><?= $esc($flash['message']) ?></div><?php endif;?>
        <?php if($queue):
            $items=self::queue($db);$pending=array_values(array_filter($items,fn($p)=>$p['moderation_status']==='pending'));
        ?>
            <section class="mod-hero page-header page-header-compact surface"><div class="page-header-main"><small class="page-header-eyebrow">EXCEPTION-BASED MODERATION</small><h2 class="page-header-title"><?= count($pending) ?> post<?= count($pending)===1?'':'s' ?> waiting for review.</h2><p class="page-header-description">Clean posts publish immediately. Only rule matches appear here. Approving publishes the original text unchanged; rejecting keeps it private.</p></div></section>
            <div class="mod-queue"><?php foreach($items as $item):?><article class="mod-review surface <?= $esc($item['moderation_status']) ?>"><header><div><small><span class="badge badge-compact <?= $item['moderation_status']==='pending'?'badge-warning':'badge-neutral' ?>"><?= $esc(strtoupper($item['moderation_status'])) ?></span> <span class="badge badge-compact <?= $item['severity']==='critical'?'badge-danger':($item['severity']==='low'?'badge-neutral':'badge-warning') ?>"><?= $esc(strtoupper((string)$item['severity'])) ?></span></small><h3>#<?= $esc($item['channel_name']) ?><?= $item['production_title']?' · '.$esc($item['production_title']):'' ?></h3><p><?= $esc($item['author']) ?> · <?= $esc($item['author_role']) ?> · <?= $esc(date('M j · g:i A',strtotime($item['created_at']))) ?></p></div><span class="badge badge-compact badge-info"><?= $esc((string)$item['category']) ?></span></header><blockquote><?= nl2br($esc($item['body'])) ?></blockquote><div class="mod-match"><b>Matched rule</b><code><?= $esc((string)$item['matched_term']) ?></code><span><?= $esc((string)$item['moderation_reason']) ?></span></div><?php if($item['moderation_status']==='pending'):?><footer><form class="toolbar-actions" method="post"><input type="hidden" name="csrf_token" value="<?= $esc((string)$_SESSION['moderation_csrf']) ?>"><input type="hidden" name="post_id" value="<?= (int)$item['id'] ?>"><button name="action" value="reject_post" class="btn btn-danger">Reject</button><button name="action" value="approve_post" class="button">Approve & publish</button></form></footer><?php else:?><footer class="mod-decided">Automatically blocked or rejected<?= $item['moderator']?' by '.$esc($item['moderator']):'' ?><?= $item['moderated_at']?' · '.$esc(date('M j · g:i A',strtotime($item['moderated_at']))):'' ?></footer><?php endif;?></article><?php endforeach;?><?php if(!$items):?><div class="mod-empty empty-state"><b>Nothing in moderation.</b><p>No Community posts have been held or blocked.</p></div><?php endif;?></div>
        <?php elseif($editing):
        ...
        ?>
            <section class="mod-edit-head page-header page-header-compact surface"><div class="page-header-main"><small class="page-header-eyebrow"><?= $edit?'RULE SETTINGS':'NEW RULE' ?></small><h2 class="page-header-title"><?= $edit?$esc($t['term']):'Add a moderation rule' ?></h2><p class="page-header-description">Normalized matching catches common substitutions, punctuation and spacing without unrestricted typo-distance matching.</p></div><div class="page-header-actions"><a href="<?= $url('/admin/moderation/terms') ?>">← Term library</a></div></section>
            <div class="mod-edit-grid"><form class="mod-form form-stack surface" method="post"><input type="hidden" name="csrf_token" value="<?= $esc((string)$_SESSION['moderation_csrf']) ?>"><input type="hidden" name="action" value="save_term"><input type="hidden" name="term_id" value="<?= (int)$t['id'] ?>"><label class="form-field">Canonical term<input class="form-input" name="term" maxlength="190" required value="<?= $esc($t['term']) ?>"></label><div class="mod-pair form-group"><label class="form-field">Category<input class="form-input" name="category" maxlength="80" required value="<?= $esc($t['category']) ?>"></label><label class="form-field">Severity<select class="form-select" name="severity"><?php foreach(['low','medium','high','critical'] as $v):?><option value="<?= $v ?>"<?= $t['severity']===$v?' selected':'' ?>><?= ucfirst($v) ?></option><?php endforeach;?></select></label></div><div class="mod-pair form-group"><label class="form-field">When matched<select class="form-select" name="rule_action"><option value="review"<?= $t['action']==='review'?' selected':'' ?>>Hold for review</option><option value="block"<?= $t['action']==='block'?' selected':'' ?>>Block immediately</option></select></label><label class="form-field">Matching<select class="form-select" name="match_mode"><option value="normalized"<?= $t['match_mode']==='normalized'?' selected':'' ?>>Normalized / fuzzy</option><option value="exact"<?= $t['match_mode']==='exact'?' selected':'' ?>>Exact</option></select></label></div><label class="form-field">Aliases <span>comma or one per line</span><textarea class="form-textarea" name="aliases" rows="4"><?= $esc(is_array($aliases)?implode("\n",$aliases):'') ?></textarea></label><label class="form-field">Admin notes<textarea class="form-textarea" name="notes" rows="3" maxlength="500"><?= $esc((string)$t['notes']) ?></textarea></label><button class="button" type="submit">Save moderation rule</button></form><aside class="mod-tester surface"><small>TEST BEFORE SAVING</small><h3>Would this text flag?</h3><form class="form-stack" method="post"><input type="hidden" name="csrf_token" value="<?= $esc((string)$_SESSION['moderation_csrf']) ?>"><input type="hidden" name="action" value="test_rule"><input type="hidden" name="term_id" value="<?= (int)$t['id'] ?>"><input type="hidden" name="term" value="<?= $esc($t['term']) ?>"><input type="hidden" name="match_mode" value="<?= $esc($t['match_mode']) ?>"><input type="hidden" name="aliases" value="<?= $esc(is_array($aliases)?implode(',',$aliases):'') ?>"><textarea class="form-textarea" name="test_text" rows="5" placeholder="Enter sample text..."><?= $test?$esc((string)$test['sample']):'' ?></textarea><button class="btn secondary" type="submit">Test current saved values</button></form><?php if($test):?><div class="mod-test-result alert <?= $test['hit']?'alert-warning hit':'alert-success clear' ?>"><b><?= $test['hit']?'FLAGGED':'CLEAR' ?></b><span><?= $test['hit']?'This sample matches the current rule.':'This sample does not match the current rule.' ?></span></div><?php endif;?></aside></div>
        <?php else:
            $terms=self::terms($db);
        ...
        ?>
            <section class="mod-list-head page-header page-header-compact surface"><div class="page-header-main"><small class="page-header-eyebrow">COMMUNITY SAFETY</small><h2 class="page-header-title">Moderation terms</h2><p class="page-header-description">Rules are grouped by type so staff can scan what CTSMD intercepts and how each match is handled.</p></div><div class="mod-hero-actions page-header-actions toolbar-actions"><a class="btn btn-secondary" href="<?= $url('/admin/moderation/queue') ?>">Review queue<?= $pending?' · '.$pending:'' ?></a><a class="button" href="<?= $url('/admin/moderation/terms/edit') ?>">Add rule</a></div></section>
            <?php if($groups):?><section class="mod-table op-table" aria-label="Moderation term library"><div class="mod-table-columns op-table-columns"><span>Term</span><span>Severity</span><span>Action</span><span>Matching</span><span>Status</span><span>Actions</span></div><?php foreach($groups as $category=>$categoryTerms):$activeCount=count(array_filter($categoryTerms,static fn(array $t):bool=>(bool)$t['active']));?><div class="mod-table-group op-table-group"><header class="op-table-group-header section-header"><div class="section-header-main"><b><?= $esc(self::categoryLabel($category)) ?></b><small><?= count($categoryTerms) ?> rule<?= count($categoryTerms)===1?'':'s' ?> · <?= $activeCount ?> active</small></div></header><?php foreach($categoryTerms as $term)self::termRow($term,$basePath,(string)$_SESSION['moderation_csrf'],$esc);?></div><?php endforeach;?></section><?php else:?><div class="mod-empty empty-state empty-state-compact"><b>No moderation rules yet.</b></div><?php endif;?>
        <?php endif;?>
