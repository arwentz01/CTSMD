...
final class ModerationExperience
{
    ...
    {
        $term=self::term($db,$id); if(!$term) throw new RuntimeException('That moderation rule could not be found.');
    ...
    private static function reviewPost(PDO $db,array $user,int $postId,bool $approve): void
    {
        ...
        $db->beginTransaction();
        try{
            $stmt=$db->prepare("SELECT cp.id,cp.moderation_status,cp.channel_id FROM channel_posts cp WHERE cp.id=:id FOR UPDATE");
            $stmt->execute(['id'=>$postId]); $post=$stmt->fetch();
            if(!$post||$post['moderation_status']!=='pending') throw new RuntimeException('Only posts awaiting review can be moderated.');
            ...
            $db->commit();
        }catch(Throwable $e){if($db->inTransaction())$db->rollBack();if($e instanceof RuntimeException)throw $e;throw new RuntimeException('The moderation decision could not be saved.');}
    }
    ...
    private static function page(string $route,string $basePath,array $user,PDO $db,?array $edit): never
    {
        ...
        $flash=$_SESSION['moderation_flash']??null;unset($_SESSION['moderation_flash']);$test=$_SESSION['moderation_test']??null;unset($_SESSION['moderation_test']);
        $editing=$route==='/admin/moderation/terms/edit';$queue=$route==='/admin/moderation/queue';
