...
final class TheatreHistoryExperience
{
    ...
    private static function page(string $basePath,array $viewer,array $subjects,?array $subject,array $credits):never
    {
        ...
        $subnav=[];if($subject){$sid=(int)$subject['id'];$subnav=[['label'=>'History','href'=>'/theatre-history?student='.$sid,'active'=>true],['label'=>'Theatre profile','href'=>'/student-profile?student='.$sid,'active'=>false],['label'=>'Acting résumé','href'=>'/theatre-history/resume?student='.$sid,'active'=>false]];}
        header('Content-Type:text/html; charset=utf-8');?><!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>My Theatre History · CTSMD Connect</title><link rel="stylesheet" href="<?=$u('/assets/css/app.css')?>"><link rel="stylesheet" href="<?=$u('/assets/css/unified-navigation.css')?>"><link rel="stylesheet" href="<?=$u('/assets/css/theatre-history.css')?>"></head><body class="app-body"><div class="unified-shell"><?php AppNavigation::renderSidebar(self::ROUTE,$basePath,$viewer);?><main class="unified-main"><?php AppNavigation::renderHeader('Your CTSMD story','Theatre History',$basePath,$subnav);?><div class="history-page">
        <section class="history-hero"><div><small>CTSMD VERIFIED CREDITS</small><h2>Every show becomes part of the story.</h2><p>Roles and participation recorded by CTSMD stay here after a production closes. Your acting résumé can combine these verified credits with clearly labeled outside experience.</p></div><div class="history-seal"><b>CTSMD</b><span>VERIFIED</span></div></section>
        ...
        <?php else:?><section class="history-heading"><div><small>THEATRE RECORD</small><h2><?=$e((string)$subject['name'])?></h2><p><?=count($credits)?> verified CTSMD credit<?=count($credits)===1?'':'s'?></p></div><a href="<?=$u('/theatre-history/resume?student='.(int)$subject['id'])?>">Open acting résumé →</a></section>
