...
final class CommunityExperience
{
    ...
    private static function createPost(PDO $db, array $user, int $channelId, string $body): void
    {
        ...
        $db->beginTransaction();
        try {
            $stmt = $db->prepare("SELECT c.id,c.production_id,c.name,c.read_scope,c.post_scope,c.read_audiences_json,c.post_audiences_json,c.archived_at,p.is_active production_active FROM channels c LEFT JOIN productions p ON p.id=c.production_id WHERE c.id=:id FOR UPDATE");
    ...
    {
        $rows = $db->query("SELECT c.id,c.production_id,c.name,c.channel_type,c.description,c.read_scope,c.post_scope,c.read_audiences_json,c.post_audiences_json,p.title production_title,p.is_active production_active,COUNT(cp.id) post_count,MAX(cp.created_at) latest_at FROM channels c LEFT JOIN productions p ON p.id=c.production_id LEFT JOIN channel_posts cp ON cp.channel_id=c.id WHERE c.archived_at IS NULL GROUP BY c.id,c.production_id,c.name,c.channel_type,c.description,c.read_scope,c.post_scope,c.read_audiences_json,c.post_audiences_json,p.title,p.is_active,c.sort_order ORDER BY c.sort_order,c.name")->fetchAll();
        $visible=[];
    ...
    {
        if($id<1) return null;
        $stmt=$db->prepare("SELECT c.id,c.production_id,c.name,c.channel_type,c.description,c.read_scope,c.post_scope,c.read_audiences_json,c.post_audiences_json,p.title production_title,p.is_active production_active FROM channels c LEFT JOIN productions p ON p.id=c.production_id WHERE c.id=:id AND c.archived_at IS NULL LIMIT 1");
    ...
    private static function canRead(PDO $db,array $user,array $channel): bool
    {
        ...
        return self::matchesAnyAudience($db,$user,$channel,self::audiences($channel,'read'));
    }

    private static function canPost(PDO $db,array $user,array $channel): bool
    {
        if(AccessPolicy::isStaff($user)) return true;
        return self::matchesAnyAudience($db,$user,$channel,self::audiences($channel,'post'));
    }
    ...
    private static function page(string $route,string $basePath,array $user,array $channels,?array $selected): never
    {
        ...
        <?php if($route==='/channels'): ?><section class="comm-hero"><small>CTSMD COMMUNITY</small><h2>The right rooms for the right people.</h2><p>Production spaces disappear automatically when a show becomes inactive. Organization-wide channels remain available, and audience rules can keep student accounts out of adult or staff spaces.</p><?php if(AccessPolicy::isStaff($user)): ?><a class="button community-manage" href="<?= $url('/admin/channels') ?>">Manage channels</a><?php endif; ?></section><div class="comm-channel-grid"><?php foreach($channels as $c): $read=self::audiences($c,'read'); ?><a class="comm-channel-card" href="<?= $url('/channels/view?id='.(int)$c['id']) ?>"><span>#</span><div><small><?= $esc($c['production_title']?:strtoupper($c['channel_type'])) ?></small><h3><?= $esc($c['name']) ?></h3><p><?= $esc($c['description']?:'Community updates and discussion.') ?></p><div class="community-policy"><em><?= $esc(self::audienceLabel($read)) ?></em><em><?= $c['can_post']?'You can post':'Read only' ?></em></div><footer><b><?= (int)$c['post_count'] ?> posts</b><em><?= $c['latest_at']?$esc(date('M j · g:i A',strtotime($c['latest_at']))):'No posts yet' ?></em></footer></div></a><?php endforeach; ?><?php if(!$channels): ?><div class="comm-empty"><b>Public community only</b><p>No additional active-production channels are available to this account.</p></div><?php endif; ?></div>
        <?php else: ?><?php if(!$selected): ?><section class="comm-empty"><b>Channel unavailable</b><p>This room may belong to an inactive production or an audience your account is not part of.</p><a class="button" href="<?= $url('/channels') ?>">Back to Community</a></section><?php else: $read=self::audiences($selected,'read'); $post=self::audiences($selected,'post'); ?><section class="comm-channel-head"><div><small><?= $esc($selected['production_title']?:strtoupper($selected['channel_type'])) ?></small><h2># <?= $esc($selected['name']) ?></h2><p><?= $esc($selected['description']?:'Community updates and discussion.') ?></p><div class="community-policy"><em>Read: <?= $esc(self::audienceLabel($read)) ?></em><em>Post: <?= $esc(self::audienceLabel($post)) ?></em></div></div><a href="<?= $url('/channels') ?>">All channels →</a></section><?php if($selected['can_post']): ?><form class="community-composer" method="post"><input type="hidden" name="csrf_token" value="<?= $esc((string)$_SESSION['community_csrf']) ?>"><input type="hidden" name="channel_id" value="<?= (int)$selected['id'] ?>"><label><span>Post to #<?= $esc($selected['name']) ?></span><textarea name="body" rows="4" maxlength="5000" required></textarea></label><footer><small>Audience: <?= $esc(self::audienceLabel($read)) ?></small><button class="button" type="submit">Publish post</button></footer></form><?php else: ?><div class="community-readonly"><b>Read-only for your account</b></div><?php endif; ?><section class="comm-feed"><?php foreach($selected['posts'] as $p): ?><article class="comm-post<?= $p['pinned']?' pinned':'' ?>"><header><i><?= $esc($p['initials']) ?></i><div><b><?= $esc($p['author']) ?></b><small><?= $esc($p['author_role']) ?> · <?= $esc(date('M j · g:i A',strtotime($p['created_at']))) ?></small></div></header><p><?= nl2br($esc($p['body'])) ?></p></article><?php endforeach; ?><?php if(!$selected['posts']): ?><div class="comm-empty"><b>No posts yet</b></div><?php endif; ?></section><?php endif; ?><?php endif; ?>
