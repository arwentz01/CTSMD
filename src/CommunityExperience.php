...
require_once __DIR__ . '/CommunityAttachmentService.php';
...
final class CommunityExperience
{
    ...
    private static function hidePost(PDO $db, array $user, int $channelId, int $postId): void
    {
        ...
        try {
            ...
            $u = $db->prepare("UPDATE channel_posts SET hidden_at=CURRENT_TIMESTAMP,hidden_by_user_id=:actor,hidden_reason='Hidden by CTSMD moderator',pinned=0,pinned_at=NULL,pinned_by_user_id=NULL WHERE id=:id AND deleted_at IS NULL");
    ...
    private static function deletePost(PDO $db, array $user, int $channelId, int $postId): void
    {
        ...
        try {
            self::postForModeration($db, $user, $channelId, $postId);
            $u = $db->prepare("UPDATE channel_posts SET deleted_at=CURRENT_TIMESTAMP,deleted_by_user_id=:actor,deleted_reason='Deleted by CTSMD moderator',hidden_at=COALESCE(hidden_at,CURRENT_TIMESTAMP),hidden_by_user_id=COALESCE(hidden_by_user_id,:actor2),pinned=0,pinned_at=NULL,pinned_by_user_id=NULL WHERE id=:id AND deleted_at IS NULL");
    ...
    private static function requireModeration(array $user): void
    {
        if (!AccessPolicy::canModerateCommunity($user)) throw new RuntimeException('Community moderation permission is required.');
    }
    ...
    private static function page(string $route, string $basePath, array $user, array $channels, ?array $selected): never
    {
        ...
        $canManage = AccessPolicy::canManageCommunity($user);
        $canModerate = AccessPolicy::canModerateCommunity($user);
