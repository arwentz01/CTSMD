...
final class StudentProfileService
{
    ...
    {
        if($studentId<1||!self::isStudent($db,$studentId))return false;if((int)$viewer['id']===$studentId)return true;if(AccessPolicy::canManagePeople($viewer))return true;$s=$db->prepare("SELECT 1 FROM family_relationships fr JOIN users student ON student.id=fr.student_user_id AND student.active=1 WHERE fr.guardian_user_id=:viewer AND fr.student_user_id=:student AND fr.status='active' LIMIT 1");$s->execute(['viewer'=>(int)$viewer['id'],'student'=>$studentId]);return(bool)$s->fetchColumn();
    }
    ...
    {
        if(self::canEdit($db,$viewer,$studentId))return true;
        $s=$db->prepare("SELECT 1
            FROM production_memberships mine
            JOIN productions p ON p.id=mine.production_id AND p.is_active=1
            JOIN production_memberships student ON student.production_id=mine.production_id
                AND student.user_id=:student
                AND student.audience_type='student'
                AND student.status='active'
            JOIN users subject ON subject.id=student.user_id AND subject.active=1
            WHERE mine.user_id=:viewer AND mine.status='active'
            LIMIT 1");
        $s->execute(['student'=>$studentId,'viewer'=>(int)$viewer['id']]);return(bool)$s->fetchColumn();
    }
    ...
    private static function isStudent(PDO $db,int $id):bool{$s=$db->prepare("SELECT 1 FROM auth_user_roles ur JOIN auth_roles r ON r.id=ur.role_id JOIN users u ON u.id=ur.user_id AND u.active=1 WHERE ur.user_id=:user AND r.code='student' LIMIT 1");$s->execute(['user'=>$id]);return(bool)$s->fetchColumn();}
