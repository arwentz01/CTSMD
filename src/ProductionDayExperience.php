...
final class ProductionDayExperience
{
    ...
    private static function page(string $basePath,array $user,?array $production,string $date,?array $data,?array $flash):never
    {
        ...
        <aside class="pd-side"><section class="pd-brief"><header><small>STAFF BRIEFING</small><h2>Day notes</h2></header><div class="pd-brief-print"><p><b>Status:</b> <?=ucfirst($e((string)($brief['day_status']??'planning')))?></p><?php if(!empty($brief['headline'])):?><p><b>Headline:</b> <?=$e((string)$brief['headline'])?></p><?php endif;?><?php if(!empty($brief['arrival_note'])):?><p><b>Arrival / access:</b><br><?=nl2br($e((string)$brief['arrival_note']))?></p><?php endif;?><?php if(!empty($brief['operations_note'])):?><p><b>Operations:</b><br><?=nl2br($e((string)$brief['operations_note']))?></p><?php endif;?></div><form method="post"><input type="hidden" name="csrf_token" value="<?=$e((string)$_SESSION['production_day_csrf'])?>"><label>Status<select name="day_status"><option value="planning"<?=($brief['day_status']??'planning')==='planning'?' selected':''?>>Planning</option><option value="live"<?=($brief['day_status']??'')==='live'?' selected':''?>>Live</option><option value="closed"<?=($brief['day_status']??'')==='closed'?' selected':''?>>Closed</option></select></label><label>Headline<input name="headline" maxlength="190" value="<?=$e((string)($brief['headline']??''))?>" placeholder="Tech rehearsal · full company"></label><label>Arrival / access note<textarea name="arrival_note" rows="3" maxlength="3000" placeholder="Parking, entrance, check-in or arrival details…"><?=$e((string)($brief['arrival_note']??''))?></textarea></label><label>Operations note<textarea name="operations_note" rows="6" maxlength="5000" placeholder="Staff-only run-of-day notes…"><?=$e((string)($brief['operations_note']??''))?></textarea></label><button type="submit">Save day briefing</button></form></section>
        <section class="pd-panel"><header><div><small>VOLUNTEERS</small><h2>Coverage</h2></div><a href="<?=$u('/admin/volunteer-shifts')?>">Manage →</a></header><?php if(!$data['volunteer_shifts']):?><p>No volunteer shifts start today.</p><?php endif;?><?php foreach($data['volunteer_shifts'] as $v):$open=max(0,(int)$v['required_slots']-(int)$v['filled_slots']);$blocked=(int)($v['eligibility_blocked']??0);?><a class="pd-mini <?=$open?'warn':''?>" href="<?=$u('/admin/volunteer-shifts/view?id='.(int)$v['id'])?>"><span><b><?=$e((string)$v['title'])?></b><small><?=date('g:i A',strtotime((string)$v['starts_at']))?> · <?=$e((string)$v['location'])?><?=$blocked?' · '.$blocked.' clearance issue'.($blocked===1?'':'s'):''?></small></span><em><?=$open?$open.' open':(int)$v['checked_in'].' checked in'?></em></a><?php endforeach;?></section>
        ...
        <?php endif;?></div></main></div><script src="<?=$u('/assets/js/unified-navigation.js')?>"></script><script>let pdPrintState=[];window.addEventListener('beforeprint',()=>{pdPrintState=[...document.querySelectorAll('.pd-call details')].map(d=>d.open);document.querySelectorAll('.pd-call details').forEach(d=>d.open=true);});window.addEventListener('afterprint',()=>{document.querySelectorAll('.pd-call details').forEach((d,i)=>d.open=!!pdPrintState[i]);pdPrintState=[];});</script></body></html><?php exit;
    }
