...
<div class="vp-shell">
    ...
    <main class="vp-main">
        ...
        <?php elseif ($route === '/production'): ?>
            <section class="vp-intro compact"><span>CURRENT PRODUCTION</span><h2>Matilda Jr.</h2><p>A single operational home for the show, with the information families need separated from the controls staff need.</p></section>
            <div class="vp-hero-grid"><article class="vp-feature-card dark"><small>TONIGHT</small><h3><?= $esc($data['schedule'][0]['title'] ?? 'Production activity') ?></h3><p><?= $esc($data['schedule'][0]['detail'] ?? 'Schedule details') ?></p><a href="<?= $url('/schedule') ?>">Open run of show →</a></article><article class="vp-feature-card"><small>PRODUCTION HEALTH</small><div class="vp-kpis"><span><b><?= count($data['announcements']) ?></b>updates</span><span><b><?= count($data['shifts']) ?></b>volunteer shifts</span><span><b><?= count($data['forms']) ?></b>assigned forms</span></div></article></div>
        ...
        <?php elseif ($route === '/people'): ?>
            <section class="vp-intro compact"><span>ADMINISTRATION</span><h2>People are relationships, not rows.</h2><p>The admin experience should make family links, roles, productions and safety context obvious before anyone opens a record.</p></section>
            ...
            <div class="vp-people-grid">
                <article class="vp-family-card"><header><div><i>JC</i><span><b>Jamie Carter</b><small>Parent · Volunteer</small></span></div><span class="pill ready">Ready</span></header><div class="vp-family-link"><span>Guardian of</span><b>Emma Carter</b><small>Current production · Student</small></div><footer><span>Volunteer credentials current</span><button>Open family</button></footer></article>
                <article class="vp-family-card"><header><div><i>MR</i><span><b>Maya Rivera</b><small>Production Manager</small></span></div><span class="pill current">Staff</span></header><div class="vp-family-link"><span>Assigned to</span><b>Matilda Jr.</b><small>Production staff · Messaging access</small></div><footer><span>Safeguarding context available</span><button>Open person</button></footer></article>
                <article class="vp-family-card"><header><div><i>TB</i><span><b>Taylor Brooks</b><small>Volunteer</small></span></div><span class="pill ready">Ready</span></header><div class="vp-family-link"><span>Eligible for</span><b>Sensitive volunteer roles</b><small>Background check + training current</small></div><footer><span>No family relationship</span><button>Open person</button></footer></article>
            </div>
        ...
        <?php elseif ($route === '/safeguarding'): ?>
            ...
            <div class="vp-kpi-row"><article><b>2</b><span>Items awaiting review</span></article><article><b>1</b><span>Conversation report</span></article><article><b>0</b><span>Guardian violations</span></article><article><b>3</b><span>Credential exceptions</span></article></div>
            <section class="vp-review-queue"><header><div><small>REVIEW QUEUE</small><h3>Needs safeguarding attention</h3></div><button>Filters</button></header><article><span class="vp-severity high">High</span><div><b>Reported safeguarded conversation</b><small>Conversation remains guardian-visible · report submitted for review</small></div><button>Review</button></article><article><span class="vp-severity medium">Review</span><div><b>Volunteer credential exception</b><small>Background check status requires administrator review</small></div><button>Review</button></article></section>

        <?php elseif ($route === '/resources'): ?>
            <section class="vp-intro compact"><span>MATILDA JR. · RESOURCE HUB</span><h2>Everything people keep asking someone to resend.</h2><p>Production resources should be findable by purpose, audience and urgency instead of buried in old posts.</p></section>
            <div class="vp-resource-grid"><article><span>★</span><small>THIS WEEK</small><h3>Tech week packet</h3><p>Call sheet, arrival map, costume checklist and family notes.</p><button>Open packet</button></article><article><span>♫</span><small>CAST</small><h3>Music & rehearsal tracks</h3><p>Current approved practice tracks organized by number.</p><button>Browse music</button></article><article><span>▶</span><small>CHOREOGRAPHY</small><h3>Review videos</h3><p>Reference videos grouped by scene and ensemble.</p><button>View videos</button></article><article><span>▤</span><small>FAMILIES</small><h3>Parent documents</h3><p>Handbook, venue information, parking and production expectations.</p><button>Browse documents</button></article></div>
        ...
        <?php elseif ($route === '/notifications'): ?>
            ...
            <div class="vp-notification-layout"><section><header><small>ACTION REQUIRED</small><h3>Needs you</h3></header><article class="vp-note urgent"><i>!</i><div><b>Emergency Information is missing</b><small>Due Aug 8 · Family form</small></div><a href="<?= $url('/forms') ?>">Complete</a></article><article class="vp-note"><i>♡</i><div><b>Volunteer shift available</b><small>Front of House · Friday evening</small></div><a href="<?= $url('/volunteer-shifts') ?>">View</a></article></section><section><header><small>WHAT CHANGED</small><h3>Updates</h3></header><?php foreach ($data['announcements'] as $announcement): ?><article class="vp-note"><i>#</i><div><b><?= $esc($announcement['title']) ?></b><small><?= $esc($announcement['meta']) ?></small></div><a href="<?= $url('/channels') ?>">Open</a></article><?php endforeach; ?></section></div>
        <?php endif; ?>
        </div>
    </main>
</div>
