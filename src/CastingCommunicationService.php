...
final class CastingCommunicationService
{
    public static function publication(PDO $db,int $productionId):array
    {
        ...
        return $s->fetch() ?: ['production_id'=>$productionId,'status'=>'draft','headline'=>null,'member_note'=>null,'published_at'=>null,'publisher_name'=>null];
    }
    ...
    public static function sendResult(PDO $db,int $productionId,int $actorId,int $recordId,string $baseUrl):int
    {
        ...
        try{
            ...
            if($r['casting_status']==='cast'&&trim((string)($r['role_title']??''))==='')throw new RuntimeException('Add the cast role before sending the result.');
            ...
            $queued=0;
            foreach($recipients as $email=>$recipient){$id=MailService::queue($db,$recipient['user_id'],$email,$recipient['name'],'system',$subject,$body,null,'casting-result-'.$recordId.'-'.$status.'-'.$email.'-'.date('YmdHi'));if($id>0)$queued++;}
    ...
    public static function savePublication(PDO $db,int $productionId,int $actorId,string $headline,string $note,bool $publish):void
    {
        ...
        if($publish){$count=self::publishableCount($db,$productionId);if($count<1)throw new RuntimeException('Finalize at least one cast member to the roster before publishing the cast.');}
        $status=$publish?'published':'draft';
        $s=$db->prepare("INSERT INTO production_cast_publications (production_id,status,headline,member_note,published_by_user_id,published_at) VALUES (:production,:status,:headline,:note,:actor,".($publish?'CURRENT_TIMESTAMP':'NULL').") ON DUPLICATE KEY UPDATE status=VALUES(status),headline=VALUES(headline),member_note=VALUES(member_note),published_by_user_id=".($publish?'VALUES(published_by_user_id)':'published_by_user_id').",published_at=".($publish?'CURRENT_TIMESTAMP':'NULL').",updated_at=CURRENT_TIMESTAMP");
        $s->execute(['production'=>$productionId,'status'=>$status,'headline'=>$headline!==''?$headline:null,'note'=>$note!==''?$note:null,'actor'=>$actorId]);
        self::audit($db,$actorId,$publish?'casting.cast_published':'casting.cast_publication_saved',$productionId,$publish?'Published production cast to members.':'Saved cast publication draft.',['headline'=>$headline?:null,'publishable_count'=>self::publishableCount($db,$productionId)]);
    }

    public static function publishedCastsForUser(PDO $db,int $userId):array
    {
        $s=$db->prepare("SELECT p.id production_id,p.title,p.season,pcp.headline,pcp.member_note,pcp.published_at FROM production_cast_publications pcp JOIN productions p ON p.id=pcp.production_id AND p.is_active=1 WHERE pcp.status='published' AND EXISTS (SELECT 1 FROM production_memberships pm WHERE pm.production_id=p.id AND pm.user_id=:user AND pm.status='active') ORDER BY pcp.published_at DESC,p.title");
        $s->execute(['user'=>$userId]);$out=[];
        foreach($s->fetchAll() as $production){$cast=$db->prepare("SELECT CONCAT(u.first_name,' ',u.last_name) name,pm.participation_role role,cr.participation_track FROM production_casting_records cr JOIN users u ON u.id=cr.user_id JOIN production_memberships pm ON pm.id=cr.production_membership_id AND pm.status='active' WHERE cr.production_id=:production AND cr.casting_status='cast' AND cr.rostered_at IS NOT NULL ORDER BY COALESCE(cr.participation_track,''),COALESCE(pm.participation_role,''),u.last_name,u.first_name");$cast->execute(['production'=>(int)$production['production_id']]);$production['cast']=$cast->fetchAll();$out[]=$production;}
        return $out;
    }
