...
final class VolunteerExperience
{
    ...
    private static function signup(PDO $db, int $userId, int $shiftId): void
    {
        $db->beginTransaction();
        try {
            $stmt = $db->prepare('SELECT id, required_slots, approval_required, starts_at FROM volunteer_shifts WHERE id = :id FOR UPDATE');
            $stmt->execute(['id' => $shiftId]);
            $shift = $stmt->fetch();
            if (!$shift) throw new RuntimeException('That volunteer shift no longer exists.');
            if (strtotime((string)$shift['starts_at']) <= time()) throw new RuntimeException('This volunteer shift has already started and can no longer accept signups.');
            ...
            if (!VolunteerCoverageService::isLiveVolunteer($db,$userId)) throw new RuntimeException('Your volunteer profile is not currently available for shift signup.');

            $missing = VolunteerCoverageService::missingRequirements($db, $userId, $shiftId);
            ...
            $filled = VolunteerCoverageService::eligibleLiveSignupCount($db,$shiftId);
            if ($filled >= (int)$shift['required_slots']) throw new RuntimeException('That shift filled before your signup was completed.');
    ...
    {
        $stmt = $db->prepare("SELECT vs.id, vs.title, vs.category, vs.starts_at, vs.ends_at, vs.location, vs.required_slots, vs.approval_required, mine.id AS my_signup_id, mine.status AS my_status FROM volunteer_shifts vs LEFT JOIN volunteer_shift_signups mine ON mine.shift_id = vs.id AND mine.user_id = :user_id WHERE vs.ends_at >= NOW() ORDER BY vs.starts_at ASC");
        $stmt->execute(['user_id' => $userId]);
        $rows = $stmt->fetchAll();
        ...
        foreach ($rows as &$row) {
            ...
            $row['actionable'] = !$row['started'] && $liveVolunteer && $row['requirements_met'] && !$row['approval_gate'];
            $row['open_slots'] = max((int)$row['required_slots'] - (int)$row['active_signups'], 0);
            $row['my_live_eligible'] = empty($row['my_signup_id']) || !in_array((string)$row['my_status'],['signed_up','checked_in'],true)
                ? true
                : VolunteerCoverageService::signupFillsLiveSlot($db,(int)$row['my_signup_id']);
        }
        unset($row);
        return $rows;
    }
    ...
    private static function page(string $route, string $basePath, array $user, array $credentials, array $shifts, ?array $selectedShift): never
    {
        ...
        $commitments = array_values(array_filter($shifts, static fn(array $s): bool => in_array($s['my_status'], ['signed_up','checked_in'], true)));
