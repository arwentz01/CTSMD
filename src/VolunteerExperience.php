...
final class VolunteerExperience
{
    ...
    private static function signup(PDO $db, int $userId, int $shiftId): void
    {
        $db->beginTransaction();
        try {
            $stmt = $db->prepare('SELECT id, required_slots FROM volunteer_shifts WHERE id = :id FOR UPDATE');
            ...
            if ($missing) {
                throw new RuntimeException('You are not eligible yet. Complete: ' . implode(', ', $missing) . '.');
            }
            ...
            if ((int)$count->fetchColumn() >= (int)$shift['required_slots']) {
                throw new RuntimeException('That shift filled before your signup was completed.');
            }
    ...
    private static function shiftById(PDO $db, int $userId, int $shiftId): ?array
    {
        ...
        foreach (self::shifts($db, $userId) as $shift) {
    ...
    {
        $_SESSION['volunteer_flash'] = ['type' => $type, 'message' => $message];
    }
    ...
    private static function page(string $route, string $basePath, array $user, array $credentials, array $shifts, ?array $selectedShift): never
    {
        ...
        $flash = $_SESSION['volunteer_flash'] ?? null;
        ...
        $eligibleCount = count(array_filter($shifts, static fn(array $s): bool => $s['eligible'] && $s['open_slots'] > 0));
        ...
        $title = match ($route) {
            '/volunteer-readiness' => 'Volunteer readiness',
            '/volunteer-shifts' => 'Volunteer opportunities',
            default => $selectedShift['title'] ?? 'Volunteer shift',
        };
...
<head>
    ...
    <link rel="stylesheet" href="<?= $url('/assets/css/volunteer-implementation.css') ?>">
</head>
<body class="app-body">
<div class="unified-shell">
    <?php AppNavigation::renderSidebar($route, $basePath, $user); ?>
    <main class="unified-main">
        <?php AppNavigation::renderHeader('Volunteer', $title, $basePath, $subnav); ?>
        <div class="vol-page">
            <?php if ($flash): ?><div class="vol-flash <?= $esc($flash['type']) ?>"><?= $esc($flash['message']) ?></div><?php endif; ?>

            <?php if ($route === '/volunteer-readiness'): ?>
                <section class="vol-hero"><div><small>YOUR VOLUNTEER PROFILE</small><h2>Know what you can help with before you browse.</h2><p>CTSMD checks required training and credentials against each shift. If something is locked, you should be able to see exactly why.</p></div><div class="vol-score"><b><?= $readyCount ?>/<?= count($credentials) ?></b><span>requirements current</span></div></section>
                <div class="vol-readiness-layout">
                    <section class="vol-panel"><header><small>REQUIREMENTS</small><h3>Your readiness</h3></header><?php foreach ($credentials as $credential): $current = $credential['status'] === 'approved' && (!$credential['expires_at'] || strtotime($credential['expires_at']) >= time()); ?><article class="vol-credential"><span><b><?= $esc($credential['name']) ?></b><small><?= $credential['expires_at'] ? 'Expires ' . $esc(date('M j, Y', strtotime($credential['expires_at']))) : $esc(ucfirst($credential['category'])) ?></small></span><em class="<?= $current ? 'good' : 'attention' ?>"><?= $current ? 'Current' : $esc(ucfirst($credential['status'])) ?></em></article><?php endforeach; ?></section>
                    <aside class="vol-panel accent"><small>OPPORTUNITIES</small><h3><?= $eligibleCount ?> shifts you can take now</h3><p>Only open shifts for which your current requirements qualify will be actionable.</p><a class="button full" href="<?= $url('/volunteer-shifts') ?>">Browse opportunities</a><?php if ($commitments): ?><div class="vol-commitment"><b><?= count($commitments) ?> current commitment<?= count($commitments) === 1 ? '' : 's' ?></b><span>Your confirmed shifts remain visible in the opportunities list.</span></div><?php endif; ?></aside>
                </div>

            <?php elseif ($route === '/volunteer-shifts'): ?>
                <section class="vol-heading"><div><small>OPEN OPPORTUNITIES</small><h2>Help where you’re cleared and needed.</h2><p>Availability, eligibility and your existing commitments are shown before you open a shift.</p></div><a href="<?= $url('/volunteer-readiness') ?>">Review readiness →</a></section>
                <div class="vol-shift-list"><?php foreach ($shifts as $shift): $signed = in_array($shift['my_status'], ['signed_up','checked_in','completed'], true); ?><a class="vol-shift-card<?= !$shift['eligible'] ? ' locked' : '' ?>" href="<?= $url('/volunteer/shift?id=' . (int)$shift['id']) ?>"><div class="vol-date"><b><?= $esc(date('M', strtotime($shift['starts_at']))) ?></b><span><?= $esc(date('j', strtotime($shift['starts_at']))) ?></span></div><div><small><?= $esc(strtoupper($shift['category'])) ?></small><h3><?= $esc($shift['title']) ?></h3><p><?= $esc(date('g:i A', strtotime($shift['starts_at']))) ?>–<?= $esc(date('g:i A', strtotime($shift['ends_at']))) ?> · <?= $esc($shift['location']) ?></p><?php if ($shift['missing']): ?><span class="vol-lock-reason">Needs <?= $esc(implode(' + ', $shift['missing'])) ?></span><?php endif; ?></div><div class="vol-shift-status"><?php if ($signed): ?><b class="signed">Signed up</b><?php elseif (!$shift['eligible']): ?><b class="locked">Locked</b><?php elseif ($shift['open_slots'] < 1): ?><b>Full</b><?php else: ?><b class="eligible"><?= (int)$shift['open_slots'] ?> open</b><?php endif; ?><span>View →</span></div></a><?php endforeach; ?></div>

            <?php else: ?>
                <?php if (!$selectedShift): ?>
                    <section class="vol-empty"><b>Shift not found</b><p>This volunteer opportunity may have been removed.</p><a class="button" href="<?= $url('/volunteer-shifts') ?>">Back to opportunities</a></section>
                <?php else: $signed = in_array($selectedShift['my_status'], ['signed_up','checked_in','completed'], true); ?>
                    <section class="vol-detail-hero"><div><small><?= $esc(strtoupper($selectedShift['category'])) ?></small><h2><?= $esc($selectedShift['title']) ?></h2><p><?= $esc(date('l, F j · g:i A', strtotime($selectedShift['starts_at']))) ?>–<?= $esc(date('g:i A', strtotime($selectedShift['ends_at']))) ?><br><?= $esc($selectedShift['location']) ?></p></div><div class="vol-capacity"><b><?= (int)$selectedShift['open_slots'] ?></b><span>open of <?= (int)$selectedShift['required_slots'] ?></span></div></section>
                    <div class="vol-detail-grid"><section class="vol-panel"><header><small>ELIGIBILITY</small><h3><?= $selectedShift['eligible'] ? 'Your requirements are current' : 'One or more requirements need attention' ?></h3></header><?php if ($selectedShift['missing']): ?><div class="vol-blocked"><b>This shift is locked for you.</b><p>Complete or renew: <?= $esc(implode(', ', $selectedShift['missing'])) ?>.</p><a href="<?= $url('/volunteer-readiness') ?>">Review readiness →</a></div><?php else: ?><div class="vol-cleared"><b>✓ You are cleared for this role.</b><p>Eligibility is re-checked by the server when you confirm signup.</p></div><?php endif; ?><div class="vol-facts"><span><b>Commitment</b><small><?= $esc(date('g:i A', strtotime($selectedShift['starts_at']))) ?>–<?= $esc(date('g:i A', strtotime($selectedShift['ends_at']))) ?></small></span><span><b>Location</b><small><?= $esc($selectedShift['location']) ?></small></span><span><b>Coverage</b><small><?= (int)$selectedShift['open_slots'] ?> remaining</small></span></div></section><aside class="vol-decision"><small>YOUR DECISION</small><?php if ($signed): ?><h3>You’re signed up.</h3><p>This commitment is reserved for you.</p><form method="post"><input type="hidden" name="csrf_token" value="<?= $esc((string)$_SESSION['volunteer_csrf']) ?>"><input type="hidden" name="shift_id" value="<?= (int)$selectedShift['id'] ?>"><input type="hidden" name="action" value="cancel"><button class="button secondary full" type="submit">Cancel signup</button></form><?php elseif (!$selectedShift['eligible']): ?><h3>Not available yet.</h3><p>Once the missing requirement is approved, eligible shifts become actionable automatically.</p><a class="button full" href="<?= $url('/volunteer-readiness') ?>">Review requirements</a><?php elseif ($selectedShift['open_slots'] < 1): ?><h3>This shift is full.</h3><p>No additional signup can be accepted right now.</p><a class="button secondary full" href="<?= $url('/volunteer-shifts') ?>">Find another shift</a><?php else: ?><h3>Ready to help?</h3><p>Confirming reserves one of the remaining volunteer slots.</p><form method="post"><input type="hidden" name="csrf_token" value="<?= $esc((string)$_SESSION['volunteer_csrf']) ?>"><input type="hidden" name="shift_id" value="<?= (int)$selectedShift['id'] ?>"><input type="hidden" name="action" value="signup"><button class="button full" type="submit">Confirm signup</button></form><?php endif; ?><a class="vol-back" href="<?= $url('/volunteer-shifts') ?>">← All opportunities</a></aside></div>
                <?php endif; ?>
            <?php endif; ?>
        </div>
    </main>
