...
final class MyAccountExperience
{
    ...
    private static function page(string $basePath, array $user, array $account, array $roles, ?array $flash): never
    {
        ...
        <section class="myaccount-card"><header><small>SECURITY</small><h3>Change password</h3></header><form method="post"><input type="hidden" name="csrf_token" value="<?=$e((string)$_SESSION['my_account_csrf'])?>"><input type="hidden" name="action" value="password"><label>Current password<input type="password" name="current_password" autocomplete="current-password" required></label><label>New password<input type="password" name="new_password" minlength="8" autocomplete="new-password" required></label><label>Confirm new password<input type="password" name="confirm_password" minlength="8" autocomplete="new-password" required></label><p class="myaccount-note">Use at least 8 characters. Changing your password invalidates any outstanding password-reset links.</p><button type="submit">Change password</button></form></section>
