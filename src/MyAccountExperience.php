...
final class MyAccountExperience
{
    ...
    private static function changePassword(PDO $db, int $userId, array $input): void
    {
        ...
        if (strlen($next) < Auth::PASSWORD_MIN_LENGTH) throw new RuntimeException('Use a new password with at least '.Auth::PASSWORD_MIN_LENGTH.' characters.');
    ...
    private static function page(string $basePath, array $user, array $account, array $roles, ?array $flash): never
    {
        ...
        ?><!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#a6192e"><title>My Account · CTSMD Connect</title><link rel="stylesheet" href="<?=$url('/assets/css/app.css')?>"><link rel="stylesheet" href="<?=$url('/assets/css/unified-navigation.css')?>"><link rel="stylesheet" href="<?=$url('/assets/css/my-account.css')?>"></head><body class="app-body"><div class="unified-shell"><?php AppNavigation::renderSidebar('/account',$basePath,$user);?><main class="unified-main"><?php AppNavigation::renderHeader('Account','My Account',$basePath);?><div class="myaccount-page"><?php if($flash):?><div class="myaccount-flash alert <?=$flash['type']==='success'?'alert-success':'alert-danger'?> alert-compact <?=$e((string)$flash['type'])?>"><?=$e((string)$flash['message'])?></div><?php endif;?>
        <section class="myaccount-hero page-header page-header-compact surface"><div class="page-header-main"><small class="page-header-eyebrow">YOUR CTSMD CONNECT ACCOUNT</small><h2 class="page-header-title"><?=$e(trim((string)$account['first_name'].' '.(string)$account['last_name']))?></h2><p class="page-header-description">Keep the basics current and manage your password without needing a staff administrator.</p></div><div class="myaccount-avatar"><?=$e((string)$account['initials'])?></div></section>
        <div class="myaccount-grid"><section class="myaccount-card surface"><header class="section-header"><div class="section-header-main"><small class="section-header-eyebrow">PROFILE</small><h3 class="section-header-title">Personal details</h3></div></header><form class="form-stack" method="post"><input type="hidden" name="csrf_token" value="<?=$e((string)$_SESSION['my_account_csrf'])?>"><input type="hidden" name="action" value="profile"><label class="form-field">First name<input class="form-input" name="first_name" maxlength="100" required value="<?=$e((string)$account['first_name'])?>"></label><label class="form-field">Last name<input class="form-input" name="last_name" maxlength="100" required value="<?=$e((string)$account['last_name'])?>"></label><label class="form-field">Email<input class="form-input" value="<?=$e((string)$account['email'])?>" disabled></label><p class="myaccount-note form-hint">Email is your sign-in identity. Email changes should use a verified change workflow rather than a simple profile edit.</p><button class="btn" type="submit">Save profile</button></form></section>
        <section class="myaccount-card surface"><header class="section-header"><div class="section-header-main"><small class="section-header-eyebrow">SECURITY</small><h3 class="section-header-title">Change password</h3></div></header><form class="form-stack" method="post"><input type="hidden" name="csrf_token" value="<?=$e((string)$_SESSION['my_account_csrf'])?>"><input type="hidden" name="action" value="password"><label class="form-field">Current password<input class="form-input" type="password" name="current_password" autocomplete="current-password" required></label><label class="form-field">New password<input class="form-input" type="password" name="new_password" minlength="8" autocomplete="new-password" required></label><label class="form-field">Confirm new password<input class="form-input" type="password" name="confirm_password" minlength="8" autocomplete="new-password" required></label><p class="myaccount-note form-hint">Use at least 8 characters. Changing your password invalidates any outstanding password-reset links.</p><button class="btn" type="submit">Change password</button></form></section>
        <section class="myaccount-card summary surface"><header class="section-header"><div class="section-header-main"><small class="section-header-eyebrow">ACCOUNT STATUS</small><h3 class="section-header-title">Access summary</h3></div></header><dl><div><dt>Membership</dt><dd><span class="badge badge-compact badge-neutral"><?=$e(ucfirst((string)$account['organization_membership_status']))?></span></dd></div><div><dt>Account</dt><dd><span class="badge badge-compact <?=$account['account_status']==='active'?'badge-success':'badge-neutral'?>"><?=$e(ucfirst((string)$account['account_status']))?></span></dd></div><div><dt>Email verified</dt><dd><span class="badge badge-compact <?=$account['email_verified_at']?'badge-success':'badge-warning'?>"><?=$account['email_verified_at']?'Yes':'No'?></span></dd></div><div><dt>Last sign-in</dt><dd><?=$account['last_login_at']?$e(date('M j, Y · g:i A',strtotime((string)$account['last_login_at']))):'Not recorded'?></dd></div><div><dt>Password changed</dt><dd><?=$account['password_changed_at']?$e(date('M j, Y',strtotime((string)$account['password_changed_at']))):'Not recorded'?></dd></div></dl><div class="myaccount-role-list"><?php foreach($roles as $role):?><span class="badge badge-compact badge-neutral"><?=$e(ucwords(str_replace('_',' ',(string)$role)))?></span><?php endforeach;?></div><a class="myaccount-link" href="<?=$url('/notification-preferences')?>">Notification preferences →</a></section></div></div></main></div><script src="<?=$url('/assets/js/unified-navigation.js')?>"></script></body></html><?php
        exit;
    }
