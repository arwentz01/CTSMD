...
final class EmailOperationsExperience
{
    ...
    public static function render(string $basePath):never
    {
        ...
        $counts=[];foreach(['queued','sending','sent','failed','suppressed'] as $status){$s=$db->prepare('SELECT COUNT(*) FROM email_queue WHERE status=:status');$s->execute(['status'=>$status]);$counts[$status]=(int)$s->fetchColumn();}
