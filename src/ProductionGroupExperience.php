...
final class ProductionGroupExperience
{
    ...
    private static function saveMembers(PDO $db,array $actor,int $productionId,int $groupId,array $membershipIds): void
    {
        ...
        if($ids){
            ...
            $stmt=$db->prepare("SELECT pm.id FROM production_memberships pm JOIN users u ON u.id=pm.user_id AND u.active=1 AND u.account_status<>'disabled' WHERE pm.production_id=? AND pm.status='active' AND pm.id IN ($ph)");
            ...
            if($valid!==$expected) throw new RuntimeException('One or more selected people are no longer available active members of this production.');
        }
        $db->beginTransaction();
        try{
    ...
    {
        $stmt=$db->prepare("SELECT pm.id membership_id,pm.user_id,pm.audience_type,pm.participation_role,CONCAT(u.first_name,' ',u.last_name) name,u.initials FROM production_memberships pm JOIN users u ON u.id=pm.user_id AND u.active=1 AND u.account_status<>'disabled' WHERE pm.production_id=:production AND pm.status='active' ORDER BY FIELD(pm.audience_type,'student','staff','guardian'),u.last_name,u.first_name");
    ...
    {
        $stmt=$db->prepare("SELECT pgm.production_membership_id FROM production_group_members pgm JOIN production_groups pg ON pg.id=pgm.group_id JOIN production_memberships pm ON pm.id=pgm.production_membership_id AND pm.production_id=pg.production_id AND pm.status='active' JOIN users u ON u.id=pm.user_id AND u.active=1 AND u.account_status<>'disabled' WHERE pgm.group_id=:group_id AND pgm.status='active'");
...
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Production Groups · CTSMD Connect</title><link rel="stylesheet" href="<?= $url('/assets/css/app.css') ?>"><link rel="stylesheet" href="<?= $url('/assets/css/unified-navigation.css') ?>"><link rel="stylesheet" href="<?= $url('/assets/css/production-groups.css') ?>"></head><body class="app-body"><div class="unified-shell"><?php AppNavigation::renderSidebar($route,$basePath,$user);?><main class="unified-main"><?php AppNavigation::renderHeader('Production','Groups & calls',$basePath,AppNavigation::productionSubnav($route,$user));?><div class="pg-page">
...
<div class="pg-layout"><section class="pg-panel"><header><div><small>PRODUCTION GROUPS</small><h3>Cast, crew & working groups</h3></div></header><div class="pg-grid op-table" aria-label="Production groups"><div class="pg-columns op-table-columns"><span>Group</span><span>Type</span><span>Members</span><span>Status</span><span>Actions</span></div><?php foreach($groups as $g):?><article class="pg-card op-table-row<?= $g['active']?'':' is-inactive'?>"><div class="pg-card-name op-table-primary"><b><?= $esc($g['name']) ?></b><small><?= $esc($g['description']?:'No description yet.') ?></small></div><div><span class="op-mobile-label">Type</span><?= $esc(self::TYPES[$g['group_type']]??ucfirst($g['group_type'])) ?></div><div><span class="op-mobile-label">Members</span><span class="badge badge-compact badge-neutral"><?= (int)$g['member_count'] ?></span></div><div><span class="op-mobile-label">Status</span><span class="badge badge-compact <?= $g['active']?'badge-success':'badge-neutral' ?>"><?= $g['active']?'Active':'Inactive' ?></span></div><div class="op-table-actions"><a class="btn btn-tertiary btn-compact" href="<?= $url('/production/groups/view?id='.(int)$g['id']) ?>">Manage</a><form method="post"><input type="hidden" name="csrf_token" value="<?= $esc((string)$_SESSION['production_group_csrf']) ?>"><input type="hidden" name="action" value="toggle"><input type="hidden" name="group_id" value="<?= (int)$g['id'] ?>"><button class="btn btn-tertiary btn-compact"><?= $g['active']?'Deactivate':'Reactivate' ?></button></form></div></article><?php endforeach;?><?php if(!$groups):?><div class="pg-empty compact op-table-empty empty-state empty-state-compact"><b>No production groups yet</b><p>Create Full Cast, Ensemble, Tech Crew, Principals, or whatever reflects how this show actually works.</p></div><?php endif;?></div></section>
...
<section class="pg-edit-head"><div><small><?= $esc(strtoupper($production['title'])) ?></small><h2><?= $esc($group['name']) ?></h2><p><?= (int)$group['member_count'] ?> available active members · <?= $esc(self::TYPES[$group['group_type']]??ucfirst($group['group_type'])) ?></p></div><a href="<?= $url('/production/groups') ?>">← All groups</a></section>
<div class="pg-layout"><section class="pg-panel"><header><div><small>MEMBERSHIP</small><h3>Who belongs in this group?</h3></div></header><form method="post" class="pg-member-form"><input type="hidden" name="csrf_token" value="<?= $esc((string)$_SESSION['production_group_csrf']) ?>"><input type="hidden" name="action" value="members"><input type="hidden" name="group_id" value="<?= (int)$group['id'] ?>"><div class="pg-roster"><?php foreach($roster as $member):?><label><input type="checkbox" name="membership_ids[]" value="<?= (int)$member['membership_id'] ?>"<?= in_array((int)$member['membership_id'],$selected,true)?' checked':'' ?>><i><?= $esc($member['initials']) ?></i><span><b><?= $esc($member['name']) ?></b><small><?= $esc(ucfirst($member['audience_type'])) ?> · <?= $esc($member['participation_role']?:'No production role') ?></small></span></label><?php endforeach;?></div><footer><small>Only available active production members can be targeted. Students' active guardians receive family-facing group calls automatically.</small><button class="button" type="submit">Save members</button></footer></form></section>
