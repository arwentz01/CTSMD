...
final class CommunityManagementExperience
{
    ...
    private static function channelRow(array $c,string $basePath,string $csrf,callable $esc): void
    {
        ...
        ?>
        <div class="ca-table-row<?= $c['archived_at']?' archived':'' ?>">
            <div class="ca-channel-name"><span>#</span><div><b><?= $esc($c['name']) ?></b><?php if($c['description']):?><small><?= $esc($c['description']) ?></small><?php endif;?></div></div>
            <div><span class="ca-mobile-label">Type</span><?= $esc(ucwords(str_replace('_',' ',(string)$c['channel_type']))) ?></div>
            <div class="ca-audience"><span class="ca-mobile-label">Audience</span><?= $esc($audience) ?></div>
            <div class="ca-number"><span class="ca-mobile-label">Posts</span><?= (int)$c['post_count'] ?></div>
            <div><span class="ca-status <?= $c['archived_at']?'archived':(!empty($c['production_id'])&&!$c['production_active']?'inactive':'live') ?>"><?= $c['archived_at']?'Archived':(!empty($c['production_id'])&&!$c['production_active']?'Inactive show':'Active') ?></span></div>
            <div class="ca-actions"><a href="<?= $url('/admin/channels/edit?id='.(int)$c['id']) ?>">Settings</a><form method="post"><input type="hidden" name="csrf_token" value="<?= $esc($csrf) ?>"><input type="hidden" name="channel_id" value="<?= (int)$c['id'] ?>"><input type="hidden" name="action" value="<?= $c['archived_at']?'restore':'archive' ?>"><button><?= $c['archived_at']?'Restore':'Archive' ?></button></form></div>
        </div>
        <?php
    }
    ...
    private static function page(string $route,string $basePath,array $user,array $channels,array $productions,?array $edit): never
    {
        ...
        ?>
        <section class="ca-list-head"><div><small>COMMUNITY STRUCTURE</small><h2>Channels</h2><p>Organization-wide rooms stay separate from production-specific communication.</p></div><a class="button" href="<?= $url('/admin/channels/edit') ?>">Create channel</a></section>
        <section class="ca-table" aria-label="Channel management">
            <div class="ca-table-columns"><span>Channel</span><span>Type</span><span>Audience</span><span>Posts</span><span>Status</span><span>Actions</span></div>
            <div class="ca-table-group"><header><div><b>Organization</b><small><?= count($org) ?> channel<?= count($org)===1?'':'s' ?></small></div></header><?php foreach($org as $c)self::channelRow($c,$basePath,(string)$_SESSION['channel_admin_csrf'],$esc);?><?php if(!$org):?><div class="ca-empty-row">No organization-wide channels.</div><?php endif;?></div>
            <?php foreach($productionGroups as $group):?><div class="ca-table-group production"><header><div><b><?= $esc((string)$group['title']) ?></b><small><?= count($group['channels']) ?> channel<?= count($group['channels'])===1?'':'s' ?></small></div><span class="ca-status <?= $group['active']?'live':'inactive' ?>"><?= $group['active']?'Active production':'Inactive production' ?></span></header><?php foreach($group['channels'] as $c)self::channelRow($c,$basePath,(string)$_SESSION['channel_admin_csrf'],$esc);?></div><?php endforeach;?>
        </section>
