...
final class ScheduleNoticeExperience
{
    ...
    public static function render(string $route,string $basePath):never
    {
        ...
        if($route==='/production/notice'){
            ...
            if($selected){$selected['group_names']=ScheduleAudience::groupNamesForItem($db,(int)$selected['schedule_item_id']);$channels=self::channels($db,(int)$selected['production_id']);$audience=ScheduleAudience::audienceMembersForItem($db,(int)$selected['schedule_item_id']);$deliveries=self::deliveries($db,(int)$selected['id']);}
        }
    ...
    private static function publish(PDO $db,array $user,int $noticeId,array $input):?string
    {
        ...
        try{
            ...
            if($sendChannel&&$notice['audience_mode']==='groups')throw new RuntimeException('Group-targeted schedule updates currently publish as targeted in-app notifications only. Choose a group/private Community room manually if you also want a channel post.');
