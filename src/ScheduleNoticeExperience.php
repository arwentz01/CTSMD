...
final class ScheduleNoticeExperience
{
    ...
    {
        $url=static fn(string $p):string=>($basePath?:'').$p;$esc=static fn(string $v):string=>htmlspecialchars($v,ENT_QUOTES,'UTF-8');$flash=$_SESSION['schedule_notice_flash']??$_SESSION['production_context_flash']??null;unset($_SESSION['schedule_notice_flash'],$_SESSION['production_context_flash']);$title=$route==='/production/notices'?'Production updates':($selected['subject']??'Schedule update');$subnav=AppNavigation::productionSubnav($route,$user);
