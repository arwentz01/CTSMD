...
final class ProductionExperience
{
    ...
    private static function saveScheduleChange(PDO $db, array $user, int $itemId, array $input): void
    {
        ...
        if (!$selectedProduction) {
            throw new RuntimeException('Select an active production before editing its schedule.');
        }
        ...
        try {
            ...
            if ((int)$before['production_id'] !== (int)$selectedProduction['id']) {
                throw new RuntimeException('That schedule item belongs to a different production workspace.');
            }
    ...
    private static function communicationCopy(array $item): string
    {
        ...
        $parts[] = 'Please review the selected production schedule in CTSMD Connect for the latest details.';
    ...
    private static function page(string $route, string $basePath, array $user, ?array $production, array $schedule, array $announcements, array $channels, array $coverage, ?array $playbill, array $notices, ?array $editItem, PDO $db): never
    {
        ...
        $flash = $_SESSION['production_flash'] ?? $_SESSION['production_context_flash'] ?? null;
        unset($_SESSION['production_flash'], $_SESSION['production_context_flash']);
        $title = match ($route) {
...
<?php if (!$production): ?>
<section class="prod-empty"><b>No active production available</b><p>Activate a production or add this account to an active production to open its workspace.</p></section>
<?php elseif ($route === '/production'): ?>
<section class="prod-hero"><div><small>WORKING PRODUCTION</small><h2><?= $esc($production['title']) ?></h2><p><?= $esc((string)$production['season']) ?> · One home for calls, schedule, communication, volunteer coverage and Playbill access.</p></div><span>Active</span></section>
<div class="prod-overview-grid"><section class="prod-panel dark"><small>NEXT UP</small><?php if ($next): ?><h3><?= $esc($next['title']) ?></h3><p><?= $esc(date('l, M j · g:i A', strtotime($next['starts_at']))) ?></p><p><?= $esc($next['location']) ?><?= $next['family_call_at'] ? ' · Family call ' . $esc(date('g:i A', strtotime($next['family_call_at']))) : '' ?></p><a href="<?= $url('/production/day') ?>">Open production day →</a><?php else: ?><h3>Nothing scheduled</h3><p>The selected production has no schedule items.</p><?php endif; ?></section><section class="prod-panel"><small>PRODUCTION PULSE</small><div class="prod-kpis"><span><b><?= count($schedule) ?></b><small>schedule items</small></span><span><b><?= (int)$coverage['open_slots'] ?></b><small>open volunteer slots</small></span><span><b><?= count($notices) ?></b><small>change drafts</small></span></div></section></div>
...
<?php if (!$editItem): ?><section class="prod-empty"><b>Schedule item not found in this production</b><p>It may belong to another active production. Switch the working production and try again.</p><a class="button" href="<?= $url('/schedule') ?>">Back to schedule</a></section><?php else: $audience = self::audienceMembers($db, (int)$production['id'], (string)$editItem['visibility']); ?>
